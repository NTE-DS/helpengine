...
public class AsynchronousSocketListener
{
    ...
    public static void ReadCallback(IAsyncResult ar)
    {
        ...
        if (bytesRead > 0)
        {
            ...
            if (content.IndexOf("<EOF>") > -1)
            {
                ...
                var command = content.Replace("<EOF>", "").Split(':');
    ...
    public static string GenTOC(TOCNode node)
    {
        var retVal = "";
        if (node.SubCount > 0)
        {
    ...
    public static int Main(String[] args)
    {
        ...
        var nsid = namespaceXML.Root.Elements("{http://schemas.nasutek.com/2015/Help5/HelpForWebExtensions}Library").FirstOrDefault(v => v.Attribute("Name").Value == args[0]);
        var hlp = new Help();
